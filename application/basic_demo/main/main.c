...
static void on_wifi_state_changed(bool connected, void *user_ctx) {
  ...
  const char *ap_ssid =
      basic_demo_wifi_is_ap_active() ? basic_demo_wifi_get_ap_ssid() : NULL;

  ESP_LOGI(TAG, "Wi-Fi state: sta_connected=%d ap_active=%d mode=%s ap_ssid=%s",
           connected, basic_demo_wifi_is_ap_active(),
           basic_demo_wifi_get_mode_string(), ap_ssid ? ap_ssid : "(none)");
...
static esp_err_t init_fatfs(void) {
  ...
  if (err != ESP_OK) {
    ...
    ESP_LOGI(TAG, "FATFS mounted total=%u used=%u", (unsigned int)total,
...
static void print_task_stack_info(void) {
  ...
  UBaseType_t count = uxTaskGetSystemState(
      ...
      sizeof(s_task_status_snapshot) / sizeof(s_task_status_snapshot[0]), NULL);

  for (UBaseType_t i = 0; i < count; i++) {
    ESP_LOGI(TAG, "Task %s  %u", s_task_status_snapshot[i].pcTaskName,
...
static void memory_monitor_task(void *arg) {
  ...
  while (1) {
    ...
    ESP_LOGI(TAG,
             "Memory: internal_free=%u bytes, internal_min_free=%u bytes, "
             "psram_free=%u bytes",
...
static void test_silkv3_qq(void) {
  const char *conv_dir = "/fatfs/conv";
  char amr_path[280] = {0};
  char wav_path[280] = {0};
  char asr_text[512] = {0};

  ESP_LOGI(TAG, "=== test_silkv3_qq: scanning %s ===", conv_dir);

  DIR *d = opendir(conv_dir);
  if (!d) {
    ESP_LOGW(TAG, "Cannot open %s, skip test", conv_dir);
    return;
  }

  struct dirent *entry;
  while ((entry = readdir(d)) != NULL) {
    if (strstr(entry->d_name, ".amr")) {
      snprintf(amr_path, sizeof(amr_path), "%s/%s", conv_dir, entry->d_name);
      break;
    }
  }
  closedir(d);
  ...
  if (amr_path[0] == '\0') {
    ESP_LOGW(TAG, "No .amr file found in %s, skip test", conv_dir);
    return;
  }

  ESP_LOGI(TAG, "Found test file: %s", amr_path);

  snprintf(wav_path, sizeof(wav_path), "/fatfs/tmp/test_decoded.wav");
  if (mkdir("/fatfs/tmp", 0777) != 0 && errno != EEXIST) {
    ESP_LOGE(TAG, "Failed to create /fatfs/tmp: %d", errno);
    return;
  }
  esp_err_t err = decode_silkv3_qq(amr_path, wav_path);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "decode_silkv3_qq failed: %s", esp_err_to_name(err));
    return;
  }
  ESP_LOGI(TAG, "Decoded WAV: %s", wav_path);

  //   err = claw_tools_asr_upload(wav_path, asr_text, sizeof(asr_text));
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "ASR upload failed: %s", esp_err_to_name(err));
  } else {
    ESP_LOGI(TAG, "ASR result: \"%s\"", asr_text);
  }

  remove(wav_path);
  ESP_LOGI(TAG, "=== test_silkv3_qq: done ===");
}
...
void app_main(void) {
  ...
  ESP_ERROR_CHECK(
      basic_demo_wifi_register_state_callback(on_wifi_state_changed, NULL));

  esp_err_t wifi_err =
      basic_demo_wifi_start(s_settings.wifi_ssid, s_settings.wifi_password);
  if (wifi_err != ESP_OK) {
    ...
    ESP_LOGW(
        ...
        basic_demo_wifi_get_ap_ssid(), basic_demo_wifi_get_ap_ip(),
        basic_demo_wifi_get_ap_ip());
  }

  ESP_ERROR_CHECK(app_claw_start(&s_settings));

  esp_err_t guard_err = claw_skill_register_deactivate_guard(
      "cap_lua_run", cap_lua_run_deactivate_guard);
  if (guard_err != ESP_OK) {

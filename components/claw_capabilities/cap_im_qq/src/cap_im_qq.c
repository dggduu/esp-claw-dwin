...
static int64_t cap_im_qq_now_ms(void) { return esp_timer_get_time() / 1000LL; }
...
static esp_err_t cap_im_qq_queue_inbound_frame(const char *frame,
                                               size_t frame_len) {
...
static esp_err_t
cap_im_qq_handle_inbound_ws_data(const esp_websocket_event_data_t *event) {
  size_t needed = 0;

  if (!event || event->op_code != 0x01 || !event->data_ptr ||
      event->data_len <= 0) {
    return ESP_ERR_INVALID_ARG;
  }
  ...
  if (event->payload_len <= 0 ||
    ...
    return cap_im_qq_queue_inbound_frame(event->data_ptr,
  ...
  memcpy(s_qq.ws_assembly.buf + s_qq.ws_assembly.len, event->data_ptr,
         (size_t)event->data_len);
  ...
  {
    esp_err_t err = cap_im_qq_queue_inbound_frame(s_qq.ws_assembly.buf,
                                                  s_qq.ws_assembly.len);
...
static void cap_im_qq_log_http_failure(const char *stage, esp_err_t err,
  ...
  ESP_LOGW(TAG, "%s failed: err=%s http=%d body=%s",
           stage ? stage : "QQ HTTP request", esp_err_to_name(err), status,
           body_snippet);
}
...
static esp_err_t cap_im_qq_get_access_token(void) {
  ...
  if (err != ESP_OK || status != 200) {
    ESP_LOGE(TAG, "QQ token request failed: err=%s http=%d body=%s",
...

  {
    ...
    strlcpy(s_qq.access_token, token_json->valuestring,
            sizeof(s_qq.access_token));
...
static esp_err_t cap_im_qq_ws_send_json(const char *json_str) {
  ...
  sent = esp_websocket_client_send_text(s_qq.ws_client, json_str, len,
                                        pdMS_TO_TICKS(1000));
...
static esp_err_t cap_im_qq_publish_inbound_text(const char *chat_id,
  ...
  return claw_event_router_publish_message("qq_gateway", "qq", chat_id, content,
...
static esp_err_t cap_im_qq_publish_attachment_event(const char *chat_id,
  ...
  snprintf(event.event_id, sizeof(event.event_id), "qq-attach-%" PRId64,
           event.timestamp_ms);
  event.text = "";
...
static esp_err_t
cap_im_qq_save_attachment(const char *chat_id, const char *sender_id,
                          const char *message_id, const char *attachment_kind,
                          const char *url, const char *original_filename,
                          const char *mime) {
  ...
  if (!cap_im_attachment_normalize_url(url, normalized_url,
                                       sizeof(normalized_url))) {
    return ESP_ERR_INVALID_ARG;
  }

  extension = cap_im_attachment_guess_extension(normalized_url,
                                                original_filename, mime);
  err = cap_im_attachment_build_saved_paths(
      s_qq.attachment_root_dir, "qq", chat_id, message_id, attachment_kind,
      extension, saved_dir, sizeof(saved_dir), saved_name, sizeof(saved_name),
      saved_path, sizeof(saved_path));
  if (err != ESP_OK) {
    return err;
  }

  err = cap_im_attachment_download_url_to_file(
      TAG, normalized_url, saved_path, s_qq.max_inbound_file_bytes, &bytes);
  if (err != ESP_OK) {
    return err;
  }

  ESP_LOGI(TAG, "start to call asr server,mime:%s", mime);

  // HOOK 点：拦截语音消息并送入ASR进行转写操作
  if (mime && (strncmp(mime, "voice", 5) == 0)) {
    char asr_text[512] = {0};
    // 调用 ASR 工具，将保存的音频文件转换为文本
    err = claw_tools_asr_transcribe(saved_path, asr_text, sizeof(asr_text));
    if (err == ESP_OK && asr_text[0] != '\0') {
      // 将识别出的文本作为普通消息发布，这样 LLM 会自动回复

      ESP_LOGI(TAG, "push asr_text:%s to text queue", asr_text);

      esp_err_t pub_err = cap_im_qq_publish_inbound_text(chat_id, sender_id,
                                                         message_id, asr_text);
      if (pub_err == ESP_OK) {
        ESP_LOGI(TAG, "Voice transcribed and published: %.64s", asr_text);
      } else {
        ESP_LOGW(TAG, "Failed to publish transcribed voice text: %s",
                 esp_err_to_name(pub_err));
      }
    } else {
      ESP_LOGW(TAG, "Voice ASR failed or empty transcription for %s",
               saved_path);
    }
  }
  ...
  if (!payload_json) {
    ESP_LOGW(TAG, "QQ attachment payload build failed: message=%s path=%s",
             message_id, saved_path);
    ESP_LOGI(TAG, "Saved QQ %s to %s (%u bytes)", attachment_kind, saved_path,
             (unsigned int)bytes);
    return ESP_OK;
  }

  err = cap_im_qq_publish_attachment_event(chat_id, sender_id, message_id,
                                           content_type, payload_json);
  free(payload_json);
  if (err != ESP_OK) {
    ESP_LOGW(TAG,
             "QQ attachment publish event failed: message=%s path=%s err=%s",
             message_id, saved_path, esp_err_to_name(err));
  }
...
static void cap_im_qq_handle_attachments(cJSON *attachments,
  ...
  if (!s_qq.enable_inbound_attachments) {
    ESP_LOGI(TAG, "QQ attachments ignored for %s: inbound attachments disabled",
             message_id);
    return;
  }
  ...
  ESP_LOGI(TAG, "QQ message %s attachment_count=%d chat=%s", message_id,
           cJSON_GetArraySize(attachments), chat_id ? chat_id : "");
  ...
  cJSON_ArrayForEach(item, attachments) {
    ...
    filename = cJSON_IsString(filename_json) ? filename_json->valuestring
                                             : cap_im_qq_basename(url);
    mime = cJSON_IsString(mime_json) ? mime_json->valuestring
    ...
    {
      esp_err_t err = cap_im_qq_save_attachment(chat_id, sender_id, message_id,
                                                kind, url, filename, mime);
      if (err != ESP_OK) {
        ESP_LOGW(TAG, "Failed to save QQ attachment message=%s err=%s url=%s",
                 message_id, esp_err_to_name(err), url);
      }
    }
  }
}

static void cap_im_qq_handle_attachment_field(cJSON *field, const char *chat_id,
                                              const char *sender_id,
                                              const char *message_id) {
...
static void cap_im_qq_handle_all_media(cJSON *data, const char *chat_id,
                                       const char *sender_id,
                                       const char *message_id) {
  static const char *const media_fields[] = {
      "attachments", "audio", "audios", "voice", "voices", "record",
      "records",     "file",  "files",  "video", "videos",
  };
  size_t i;
...
static void cap_im_qq_handle_dispatch(cJSON *data, const char *event_type) {
  ...
  if (strcmp(event_type, "C2C_MESSAGE_CREATE") == 0) {
    ...
    cJSON *author = cJSON_GetObjectItem(data, "author");
    cJSON *member =
        author ? cJSON_GetObjectItem(author, "member_openid") : NULL;
    cJSON *content_json = cJSON_GetObjectItem(data, "content");
    cJSON *id_json = cJSON_GetObjectItem(data, "id");
  ...
  if (content && content[0]) {
    if (cap_im_qq_publish_inbound_text(chat_id, sender_id, message_id,
                                       content) == ESP_OK) {
      ESP_LOGI(TAG, "QQ inbound %s: %.48s%s", chat_id, content,
               strlen(content) > 48 ? "..." : "");
    } else {
...
static void cap_im_qq_log_stack_watermark(const char *label) {
  ...
  ESP_LOGD(TAG, "%s stack_high_water=%u bytes", label,
           (unsigned int)(words * sizeof(StackType_t)));
}
...
static void cap_im_qq_process_frame(const char *frame, size_t frame_len) {
  ...
  case CAP_IM_QQ_WS_OP_HELLO:
    if (cJSON_IsObject(data_json)) {
      cJSON *heartbeat_json =
          cJSON_GetObjectItem(data_json, "heartbeat_interval");
...
static void cap_im_qq_inbound_task(void *arg) {
  ...
  while (1) {
    if (xQueueReceive(s_qq.inbound_queue, &item, pdMS_TO_TICKS(200)) ==
        pdTRUE) {
...
static void cap_im_qq_ws_event_handler(void *arg, esp_event_base_t base,
  ...
  if (event_id != WEBSOCKET_EVENT_DATA || !event || event->op_code != 0x01 ||
      !event->data_ptr || event->data_len <= 0) {
    return;
  }
  ...
  if (s_qq.inbound_queue) {
    ...
    if (err != ESP_OK && err != ESP_ERR_INVALID_ARG) {
      ESP_LOGW(TAG,
               "QQ inbound WS frame handling failed len=%d payload_len=%d "
               "offset=%d err=%s",
...
static esp_err_t cap_im_qq_api_post(const char *path, const char *body_json,
                                    char **out_response) {
  int attempt = 0;
...
static char *cap_im_qq_build_chat_path(const char *chat_id,
                                       const char *suffix) {
...
static char *cap_im_qq_file_to_base64(const char *path) {
  ...
  if (mbedtls_base64_encode(encoded_buf, encoded_len, &encoded_len, raw_buf,
                            read_size) != 0) {
    free(raw_buf);
    free(encoded_buf);
    return NULL;
...
static esp_err_t cap_im_qq_upload_media(const char *chat_id, const char *path,
                                        uint32_t file_type,
                                        char **out_file_info) {
  struct stat st;
  ...
  free(json_str);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "QQ media upload failed for %s: %s", chat_id,
             esp_err_to_name(err));
    free(response);
    return err;
  }
...
static esp_err_t cap_im_qq_send_media(const char *chat_id, const char *path,
                                      const char *caption, uint32_t file_type,
                                      const char *kind) {
  char *file_info = NULL;
  ...
  err = cap_im_qq_send_media_message(
      chat_id, file_info, resolved_caption ? resolved_caption : caption);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "QQ %s send failed for %s: %s", kind, chat_id,
             esp_err_to_name(err));
...
static void cap_im_qq_ws_task(void *arg) {
  ...
  while (!s_qq.stop_requested) {
    ...
    s_qq.ws_identify_pending = false;
    s_qq.ws_should_reconnect = false;
    esp_websocket_register_events(s_qq.ws_client, WEBSOCKET_EVENT_ANY,
                                  cap_im_qq_ws_event_handler, NULL);
    ...
    while (s_qq.ws_client && !s_qq.stop_requested) {
      ...
      if (!esp_websocket_client_is_connected(s_qq.ws_client) &&
          !s_qq.ws_connected &&
...
static esp_err_t cap_im_qq_gateway_start(void) {
  ...
  ok = claw_task_create(
      ...
      cap_im_qq_inbound_task, NULL, &s_qq.inbound_task);
  if (ok != pdPASS) {
  ...
  ok = claw_task_create(
      ...
      cap_im_qq_ws_task, NULL, &s_qq.ws_task);
  if (ok != pdPASS) {
...
static esp_err_t
cap_im_qq_send_message_execute(const char *input_json,
  ...
  if (cJSON_IsString(message_json) && message_json->valuestring &&
      message_json->valuestring[0]) {
  ...
  if (!chat_id || !message) {
    ESP_LOGW(TAG,
             ...
             chat_id ? chat_id : "(null)", message ? "true" : "false",
  ...
  ESP_LOGI(TAG, "QQ outbound text to %s: %.48s%s", chat_id, message,
           strlen(message) > 48 ? "..." : "");
...
static esp_err_t cap_im_qq_send_media_execute(
    const char *input_json, const claw_cap_call_context_t *ctx, char *output,
    size_t output_size, uint32_t file_type, const char *kind) {
  cJSON *root = NULL;
  cJSON *chat_id_json;
  cJSON *path_json;
  cJSON *caption_json;
  ...
  if (cJSON_IsString(path_json) && path_json->valuestring &&
...
static esp_err_t
cap_im_qq_send_image_execute(const char *input_json,
                             const claw_cap_call_context_t *ctx, char *output,
                             size_t output_size) {
  return cap_im_qq_send_media_execute(input_json, ctx, output, output_size,
                                      CAP_IM_QQ_FILE_TYPE_IMAGE, "image");
}
...
esp_err_t cap_im_qq_set_credentials(const char *app_id,
...
esp_err_t
cap_im_qq_set_attachment_config(const cap_im_qq_attachment_config_t *config) {
  if (!config) {
    return ESP_ERR_INVALID_ARG;
  }
...
esp_err_t cap_im_qq_send_image(const char *chat_id, const char *path,
                               const char *caption) {
  return cap_im_qq_send_media(chat_id, path, caption, CAP_IM_QQ_FILE_TYPE_IMAGE,
                              "image");
}

esp_err_t cap_im_qq_send_file(const char *chat_id, const char *path,
                              const char *caption) {
  return cap_im_qq_send_media(chat_id, path, caption, CAP_IM_QQ_FILE_TYPE_FILE,
                              "file");
}

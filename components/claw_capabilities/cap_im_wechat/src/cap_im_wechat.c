...
#define CAP_IM_WECHAT_DEFAULT_CDN_BASE_URL                                     \
...
static void cap_im_wechat_init_defaults(cap_im_wechat_state_t *state) {
  ...
  strlcpy(state->base_url, CAP_IM_WECHAT_DEFAULT_BASE_URL,
          sizeof(state->base_url));
  strlcpy(state->cdn_base_url, CAP_IM_WECHAT_DEFAULT_CDN_BASE_URL,
  ...
  strlcpy(state->client_version, CAP_IM_WECHAT_DEFAULT_CLIENT_VERSION,
...
static esp_err_t cap_im_wechat_lock(void) {
  ...
  return xSemaphoreTake(s_wechat.lock, pdMS_TO_TICKS(1000)) == pdTRUE
             ? ESP_OK
             : ESP_ERR_TIMEOUT;
}
...
static void cap_im_wechat_context_remember(const char *chat_id,
  ...
  for (i = 0; i < CAP_IM_WECHAT_CONTEXT_CACHE_SIZE; i++) {
    if (strcmp(s_wechat.context_cache[i].chat_id, chat_id) == 0) {
      strlcpy(s_wechat.context_cache[i].context_token, context_token,
  ...
  strlcpy(s_wechat.context_cache[slot].chat_id, chat_id,
          sizeof(s_wechat.context_cache[slot].chat_id));
  strlcpy(s_wechat.context_cache[slot].context_token, context_token,
...
static esp_err_t
cap_im_wechat_http_event_handler(esp_http_client_event_t *event) {
...
static esp_err_t
cap_im_wechat_resp_prepare(cap_im_wechat_http_resp_t *response) {
  if (!response) {
    return ESP_OK;
  }
...
static esp_err_t cap_im_wechat_build_x_wechat_uin(char *buf, size_t buf_size) {
  ...
  ret = mbedtls_base64_encode(encoded, sizeof(encoded), &out_len,
...
static esp_err_t cap_im_wechat_http_request(
    const char *url, esp_http_client_method_t method, const char *content_type,
    const void *body, size_t body_len, int timeout_ms, bool use_common_headers,
    bool use_auth_headers, cap_im_wechat_http_resp_t *response) {
  ...
  ESP_RETURN_ON_ERROR(cap_im_wechat_resp_prepare(response), TAG,
  ...
  if (use_common_headers) {
    ...
    if (s_wechat.client_version[0]) {
      esp_http_client_set_header(client, "iLink-App-ClientVersion",
                                 s_wechat.client_version);
    }
...
static esp_err_t cap_im_wechat_api_post(const char *endpoint, cJSON *root,
                                        int timeout_ms,
  ...
  if (snprintf(url, sizeof(url), "%s/%s", s_wechat.base_url, endpoint) >=
      (int)sizeof(url)) {
  ...
  err = cap_im_wechat_http_request(url, HTTP_METHOD_POST, "application/json",
                                   body_json, strlen(body_json), timeout_ms,
                                   true, true, response);
  free(body_json);
  return err;
}
...
static esp_err_t
cap_im_wechat_http_get_json(const char *base_url, const char *endpoint,
                            int timeout_ms,
                            cap_im_wechat_http_resp_t *response) {
  char url[CAP_IM_WECHAT_URL_BUF_SIZE];
  ...
  if (snprintf(url, sizeof(url), "%s/%s", base_url, endpoint) >=
      (int)sizeof(url)) {
    return ESP_ERR_INVALID_SIZE;
  }

  return cap_im_wechat_http_request(url, HTTP_METHOD_GET, "application/json",
                                    NULL, 0, timeout_ms, true, false, response);
}
...
static esp_err_t cap_im_wechat_add_base_info(cJSON *root) {
  ...
  if (!cJSON_AddStringToObject(base_info, "channel_version",
                               "esp-claw-wechat")) {
    return ESP_ERR_NO_MEM;
  }
...
static esp_err_t cap_im_wechat_publish_inbound_text(const char *chat_id,
  ...
  return claw_event_router_publish_message("wechat_gateway", "wechat", chat_id,
                                           content, sender_id, message_id);
}

static esp_err_t cap_im_wechat_publish_attachment_event(
    const char *chat_id, const char *sender_id, const char *message_id,
    const char *content_type, const char *payload_json) {
  ...
  snprintf(event.event_id, sizeof(event.event_id), "wechat-attach-%" PRId64,
           event.timestamp_ms);
  event.text = "";
...
static esp_err_t cap_im_wechat_download_buffer(const char *url,
  ...
  err = cap_im_wechat_http_request(url, HTTP_METHOD_GET, "", NULL, 0, 30000,
...
static esp_err_t cap_im_wechat_hex_decode(const char *hex, unsigned char *out,
...
static esp_err_t cap_im_wechat_parse_aes_key(const char *aes_key_base64,
  ...
  if (decoded_len == 32 && cap_im_wechat_is_hex_string(decoded, decoded_len)) {
    return cap_im_wechat_hex_decode((const char *)decoded, key_buf,
...
static esp_err_t cap_im_wechat_aes_ecb_crypt(
    const unsigned char *input, size_t input_len, const unsigned char *key,
    size_t key_len, bool encrypt, unsigned char **out_buf, size_t *out_len) {
  ...
  ret = encrypt ? mbedtls_aes_setkey_enc(&aes, key, 128)
  ...
  for (i = 0; i < padded_len; i += 16) {
    ret = mbedtls_aes_crypt_ecb(
        &aes, encrypt ? MBEDTLS_AES_ENCRYPT : MBEDTLS_AES_DECRYPT, buffer + i,
        buffer + i);
    if (ret != 0) {
      mbedtls_aes_free(&aes);
      free(buffer);
...
static esp_err_t cap_im_wechat_base64_encode(const unsigned char *input,
  ...
  ret = mbedtls_base64_encode((unsigned char *)out, out_size, &out_len, input,
                              input_len);
...
static esp_err_t cap_im_wechat_download_media_plaintext(
    const char *full_url, const char *encrypted_query_param,
    const char *aes_key_base64, size_t max_bytes, unsigned char **out_buf,
    size_t *out_len) {
  char *url = NULL;
  ...
  if (encrypted_query_param && encrypted_query_param[0]) {
    ...
    if (asprintf(&fallback_url, "%s/download?encrypted_query_param=%s",
                 s_wechat.cdn_base_url, encoded_param) < 0) {
      free(encoded_param);
      return ESP_ERR_NO_MEM;
    }
    free(encoded_param);
  ...
  err = cap_im_wechat_download_buffer(url, max_bytes + 32, &encrypted_buf,
                                      &encrypted_len);
  free(url);
  if (err != ESP_OK) {
    if (full_url && full_url[0] && fallback_url) {
      ESP_LOGW(
          TAG,
          "download via full_url failed, retrying encrypted_query_param URL");
      err = cap_im_wechat_download_buffer(fallback_url, max_bytes + 32,
  ...
  err = cap_im_wechat_parse_aes_key(aes_key_base64, key_buf, sizeof(key_buf),
                                    &key_len);
  if (err != ESP_OK) {
    free(encrypted_buf);
    return err;
  ...
  err = cap_im_wechat_aes_ecb_crypt(encrypted_buf, encrypted_len, key_buf,
                                    key_len, false, out_buf, out_len);
  free(encrypted_buf);
  if (err != ESP_OK) {
    return err;
  }
...
static esp_err_t cap_im_wechat_save_attachment_buffer(
    const char *chat_id, const char *sender_id, const char *message_id,
    const char *attachment_kind, const char *content_type,
    const char *original_filename, const unsigned char *buf, size_t buf_len) {
  ...
  extension = cap_im_attachment_guess_extension(
      original_filename, original_filename, content_type);
  err = cap_im_attachment_build_saved_paths(
      s_wechat.attachment_root_dir, "wechat", chat_id, message_id,
      attachment_kind, extension ? extension : ".bin", saved_dir,
      sizeof(saved_dir), saved_name, sizeof(saved_name), saved_path,
      sizeof(saved_path));
  if (err != ESP_OK) {
    return err;
  }
  ...
  ESP_RETURN_ON_ERROR(
      cap_im_attachment_save_buffer_to_file(TAG, saved_path, buf, buf_len), TAG,
  ...
  err = cap_im_wechat_publish_attachment_event(chat_id, sender_id, message_id,
                                               content_type, payload_json);
  free(payload_json);
  return err;
}
...
static const char *cap_im_wechat_item_aes_key(cJSON *item) {
  ...
  if (image_item) {
    value = cap_im_wechat_string_value(
        cJSON_GetObjectItemCaseSensitive(image_item, "aeskey"));
    if (value && value[0]) {
      static char encoded[96];
      unsigned char decoded[16];
      size_t decoded_len = 0;

      if (cap_im_wechat_hex_decode(value, decoded, sizeof(decoded),
                                   &decoded_len) == ESP_OK &&
          cap_im_wechat_base64_encode(decoded, decoded_len, encoded,
    ...
    if (media) {
      return cap_im_wechat_string_value(
          cJSON_GetObjectItemCaseSensitive(media, "aes_key"));
    }
  }
...
static const char *cap_im_wechat_item_full_url(cJSON *item) {
  ...
  return cap_im_wechat_string_value(
      cJSON_GetObjectItemCaseSensitive(media, "full_url"));
}
...
static const char *cap_im_wechat_item_file_name(cJSON *item) {
  ...
  return cap_im_wechat_string_value(
      cJSON_GetObjectItemCaseSensitive(file_item, "file_name"));
}
...
static esp_err_t cap_im_wechat_process_media_item(cJSON *item,
  ...
  type = cap_im_wechat_int_value(cJSON_GetObjectItemCaseSensitive(item, "type"),
                                 0);
  switch (type) {
  case 2:
    kind = "image";
  ...
  err = cap_im_wechat_download_media_plaintext(
      full_url, download_param, aes_key, s_wechat.max_inbound_file_bytes, &buf,
      &buf_len);
  if (err != ESP_OK) {
  ...
  if (type == 3) { // 语音消息
    char asr_text[512] = {0};
    esp_err_t asr_err = claw_tools_asr_transcribe_buffer(
        buf, buf_len, "silk", asr_text, sizeof(asr_text));
    if (asr_err == ESP_OK && asr_text[0] != '\0') {
      // 发布转写后的文本（需要实现对应的微信文本发布函数）
      esp_err_t pub_err = cap_im_wechat_publish_inbound_text(
          chat_id, sender_id, message_id, asr_text);
      if (pub_err == ESP_OK) {
        ESP_LOGI(TAG, "WeChat voice transcribed: %.64s", asr_text);
      } else {
        ESP_LOGW(TAG, "Failed to publish transcribed voice: %s",
                 esp_err_to_name(pub_err));
      }
    } else {
      ESP_LOGW(TAG, "WeChat voice ASR failed or empty");
    }
  }

  err = cap_im_wechat_save_attachment_buffer(chat_id, sender_id, message_id,
                                             kind, content_type, file_name, buf,
                                             buf_len);
  free(buf);
  return err;
}

static void cap_im_wechat_append_text(char *buf, size_t buf_size,
                                      const char *text) {
...
static esp_err_t cap_im_wechat_process_message(cJSON *msg) {
  ...
  from_user_id = cap_im_wechat_string_value(
      cJSON_GetObjectItemCaseSensitive(msg, "from_user_id"));
  group_id = cap_im_wechat_string_value(
      cJSON_GetObjectItemCaseSensitive(msg, "group_id"));
  context_token = cap_im_wechat_string_value(
  ...
  snprintf(message_id_buf, sizeof(message_id_buf), "%" PRId64,
           cap_im_wechat_int64_value(
               cJSON_GetObjectItemCaseSensitive(msg, "message_id"),
  ...
  for (i = 0; i < item_count; i++) {
    ...
    type = cap_im_wechat_int_value(
        cJSON_GetObjectItemCaseSensitive(item, "type"), 0);
    if (type == 1) {
      ...
      cap_im_wechat_process_media_item(item, chat_id, from_user_id,
                                       message_id_buf);
    }
  }

  {
    esp_err_t err = cap_im_wechat_publish_inbound_text(
        chat_id, from_user_id ? from_user_id : chat_id, message_id_buf,
        text_buf);
    free(text_buf);
    return err;
  }
...
static esp_err_t cap_im_wechat_poll_once(void) {
  ...
  if (!cJSON_AddStringToObject(root, "get_updates_buf",
                               s_wechat.sync_buf ? s_wechat.sync_buf : "")) {
    cJSON_Delete(root);
    return ESP_ERR_NO_MEM;
  }
  ...
  err = cap_im_wechat_api_post("ilink/bot/getupdates", root,
                               s_wechat.poll_timeout_ms + 5000, &response);
  cJSON_Delete(root);
  if (err != ESP_OK) {
    cap_im_wechat_resp_cleanup(&response);
    return err;
  ...
  if (cap_im_wechat_int_value(cJSON_GetObjectItemCaseSensitive(root, "ret"),
                              0) != 0 ||
      cap_im_wechat_int_value(cJSON_GetObjectItemCaseSensitive(root, "errcode"),
                              0) != 0) {
  ...
  s_wechat.poll_timeout_ms = cap_im_wechat_int_value(
      cJSON_GetObjectItemCaseSensitive(root, "longpolling_timeout_ms"),
  ...
  {
    const char *next_sync = cap_im_wechat_string_value(
...
static esp_err_t cap_im_wechat_send_text_chunk(const char *chat_id,
                                               const char *chunk) {
  cJSON *msg = NULL;
  cJSON *item_list = NULL;
  cJSON *item = NULL;
...
static esp_err_t cap_im_wechat_md5_hex(const unsigned char *buf, size_t len,
...
static esp_err_t cap_im_wechat_random_hex(unsigned char *raw, size_t raw_len,
...
static esp_err_t
cap_im_wechat_upload_ciphertext(const char *upload_full_url,
                                const char *upload_param, const char *filekey,
                                const unsigned char *ciphertext,
                                size_t ciphertext_len, char **encrypted_param) {
  cap_im_wechat_http_resp_t response = {0};
  char *url = NULL;
  ...
  if (upload_full_url && upload_full_url[0]) {
    ...
    if (asprintf(&url, "%s/upload?encrypted_query_param=%s&filekey=%s",
                 s_wechat.cdn_base_url, encoded_upload_param,
                 encoded_filekey) < 0) {
      free(encoded_upload_param);
  ...
  err = cap_im_wechat_http_request(
      url, HTTP_METHOD_POST, "application/octet-stream", ciphertext,
      ciphertext_len, 30000, false, false, &response);
  free(url);
  if (err == ESP_OK) {
...
static esp_err_t
cap_im_wechat_upload_file(const char *chat_id, const char *file_path,
                          int media_type, char **download_param,
                          char *aes_key_base64, size_t aes_key_base64_size,
                          size_t *ciphertext_size, size_t *plaintext_size) {
  ...
  if (!chat_id || !file_path || !download_param || !aes_key_base64 ||
      !ciphertext_size || !plaintext_size) {
    return ESP_ERR_INVALID_ARG;
  }

  ESP_RETURN_ON_ERROR(
      cap_im_wechat_read_file(file_path, &plaintext, &plain_len), TAG,
      "read upload file failed");
  ESP_RETURN_ON_ERROR(
      cap_im_wechat_md5_hex(plaintext, plain_len, md5_hex, sizeof(md5_hex)),
      TAG, "md5 failed");
  ESP_RETURN_ON_ERROR(cap_im_wechat_random_hex(aes_key_raw, sizeof(aes_key_raw),
                                               aes_key_hex,
                                               sizeof(aes_key_hex)),
                      TAG, "random aes key failed");
  ESP_RETURN_ON_ERROR(
      cap_im_wechat_base64_encode((const unsigned char *)aes_key_hex,
                                  strlen(aes_key_hex), upload_aes_key_base64,
                                  sizeof(upload_aes_key_base64)),
      TAG, "base64 aes key failed");
  {
    unsigned char filekey_raw[16];

    ESP_RETURN_ON_ERROR(
        cap_im_wechat_random_hex(filekey_raw, sizeof(filekey_raw), filekey_hex,
                                 sizeof(filekey_hex)),
        TAG, "random filekey failed");
  }
  ...
  cJSON_AddNumberToObject(root, "filesize",
                          (double)(((plain_len / 16) + 1) * 16));
  ...
  err =
      cap_im_wechat_api_post("ilink/bot/getuploadurl", root, 15000, &response);
  cJSON_Delete(root);
  if (err != ESP_OK) {
    free(plaintext);
    cap_im_wechat_resp_cleanup(&response);
  ...
  upload_full_url =
      strdup(cap_im_wechat_string_value(
                 ...
                 ? cap_im_wechat_string_value(cJSON_GetObjectItemCaseSensitive(
                       root, "upload_full_url"))
                 : "");
  upload_param =
      strdup(cap_im_wechat_string_value(
  ...
  err = cap_im_wechat_aes_ecb_crypt(plaintext, plain_len, aes_key_raw,
                                    sizeof(aes_key_raw), true, &ciphertext,
                                    &cipher_len);
  free(plaintext);
  if (err != ESP_OK) {
    free(upload_full_url);
  ...
  err = cap_im_wechat_upload_ciphertext(
      upload_full_url[0] ? upload_full_url : NULL,
      upload_param[0] ? upload_param : NULL, filekey_hex, ciphertext,
      cipher_len, download_param);
  free(upload_full_url);
  free(upload_param);
  free(ciphertext);
  if (err != ESP_OK) {
...
static esp_err_t
cap_im_wechat_send_message_execute(const char *input_json,
  ...
  chat_id = cap_im_wechat_string_value(
      cJSON_GetObjectItemCaseSensitive(root, "chat_id"));
  message = cap_im_wechat_string_value(
...
static esp_err_t
cap_im_wechat_send_image_execute(const char *input_json,
                                 const claw_cap_call_context_t *ctx,
                                 char *output, size_t output_size) {
  cJSON *root = NULL;
  const char *chat_id = NULL;
  ...
  chat_id = cap_im_wechat_string_value(
      cJSON_GetObjectItemCaseSensitive(root, "chat_id"));
  path = cap_im_wechat_string_value(
      cJSON_GetObjectItemCaseSensitive(root, "path"));
  caption = cap_im_wechat_string_value(
      cJSON_GetObjectItemCaseSensitive(root, "caption"));
  if (!chat_id || !path) {
    cJSON_Delete(root);
    return ESP_ERR_INVALID_ARG;
  }
...
static esp_err_t cap_im_wechat_gateway_start(void) {
  ...
  ok = claw_task_create(
      ...
      cap_im_wechat_poll_task, NULL, &s_wechat.poll_task);
  if (ok != pdPASS) {
    s_wechat.poll_task = NULL;
    return ESP_FAIL;
  }
...
static const claw_cap_descriptor_t s_wechat_descriptors[] = {
    {
        ...
        .cap_flags =
            CLAW_CAP_FLAG_EMITS_EVENTS | CLAW_CAP_FLAG_SUPPORTS_LIFECYCLE,
    ...
    },
    {
        ...
        .input_schema_json =
            "{\"type\":\"object\",\"properties\":{\"chat_id\":{\"type\":"
    ...
    },
    {
        ...
        .input_schema_json =
            "{\"type\":\"object\",\"properties\":{\"chat_id\":{\"type\":"
            "\"string\"},\"path\":{\"type\":\"string\"},\"caption\":{\"type\":"
            "\"string\"}},\"required\":[\"chat_id\",\"path\"]}",
...
static const claw_cap_group_t s_wechat_group = {
    ...
    .descriptor_count =
        sizeof(s_wechat_descriptors) / sizeof(s_wechat_descriptors[0]),
};
...
esp_err_t
cap_im_wechat_set_client_config(const cap_im_wechat_client_config_t *config) {
  ...
  strlcpy(s_wechat.cdn_base_url,
          (config->cdn_base_url && config->cdn_base_url[0])
              ? config->cdn_base_url
              : CAP_IM_WECHAT_DEFAULT_CDN_BASE_URL,
          sizeof(s_wechat.cdn_base_url));
  strlcpy(s_wechat.account_id,
          (config->account_id && config->account_id[0]) ? config->account_id
                                                        : "default",
  ...
  strlcpy(s_wechat.client_version,
          (config->client_version && config->client_version[0])
...
esp_err_t cap_im_wechat_set_attachment_config(
  ...
  strlcpy(s_wechat.attachment_root_dir, config->storage_root_dir,
...
esp_err_t cap_im_wechat_start(void) { return cap_im_wechat_gateway_start(); }

esp_err_t cap_im_wechat_stop(void) { return cap_im_wechat_gateway_stop(); }
...
esp_err_t cap_im_wechat_qr_login_start(const char *account_id, bool force) {
  ...
  cap_im_wechat_random_session_key(s_wechat.qr.session_key,
                                   sizeof(s_wechat.qr.session_key));
  err = cap_im_wechat_qr_fetch_code_locked();
  if (err != ESP_OK) {
    s_wechat.qr.active = false;
    strlcpy(s_wechat.qr.status, "error", sizeof(s_wechat.qr.status));
    snprintf(s_wechat.qr.message, sizeof(s_wechat.qr.message),
             "拉取二维码失败: %s", esp_err_to_name(err));
    cap_im_wechat_unlock();
    return err;
  }
  ...
  if (!s_wechat.qr_task) {
    ok = claw_task_create(
        ...
        cap_im_wechat_qr_task, NULL, &s_wechat.qr_task);
    if (ok != pdPASS) {
      s_wechat.qr.active = false;
...
esp_err_t
cap_im_wechat_qr_login_get_status(cap_im_wechat_qr_login_status_t *out_status) {
  esp_err_t err;
  ...
  strlcpy(out_status->session_key, s_wechat.qr.session_key,
          sizeof(out_status->session_key));
  strlcpy(out_status->status,
          s_wechat.qr.status[0] ? s_wechat.qr.status : "idle",
          sizeof(out_status->status));
  strlcpy(out_status->message, s_wechat.qr.message,
          sizeof(out_status->message));
  strlcpy(out_status->qr_data_url, s_wechat.qr.qr_data_url,
          sizeof(out_status->qr_data_url));
  strlcpy(out_status->account_id,
          s_wechat.qr.ilink_bot_id[0] ? s_wechat.qr.ilink_bot_id
                                      : s_wechat.qr.account_id,
          sizeof(out_status->account_id));
  strlcpy(out_status->user_id, s_wechat.qr.ilink_user_id,
          sizeof(out_status->user_id));
  strlcpy(out_status->token, s_wechat.qr.bot_token, sizeof(out_status->token));
  strlcpy(out_status->base_url,
          s_wechat.qr.base_url[0] ? s_wechat.qr.base_url
                                  : CAP_IM_WECHAT_DEFAULT_BASE_URL,
...
esp_err_t cap_im_wechat_send_image(const char *chat_id, const char *path,
                                   const char *caption) {
  ...
  if (caption && caption[0]) {
    ESP_RETURN_ON_ERROR(cap_im_wechat_send_text(chat_id, caption), TAG,
                        "send caption failed");
  }

  err = cap_im_wechat_upload_file(chat_id, path, 1, &download_param,
                                  aes_key_base64, sizeof(aes_key_base64),
                                  &ciphertext_size, &plaintext_size);
  if (err != ESP_OK) {
    return err;
  }
  ...
  (void)plaintext_size;
  err = cap_im_wechat_send_image_message(chat_id, download_param,
                                         aes_key_base64, ciphertext_size);
  free(download_param);
  return err;
}
...
static esp_err_t cap_im_wechat_qr_fetch_code_locked(void) {
  ...
  err =
      cap_im_wechat_http_get_json(CAP_IM_WECHAT_DEFAULT_BASE_URL, endpoint,
                                  CAP_IM_WECHAT_QR_START_TIMEOUT_MS, &response);
  if (err != ESP_OK) {
    cap_im_wechat_resp_cleanup(&response);
    return err;
  }
  ...
  qrcode = cap_im_wechat_string_value(
      cJSON_GetObjectItemCaseSensitive(root, "qrcode"));
  ...
  strlcpy(s_wechat.qr.qr_data_url, qrcode_img_content,
          sizeof(s_wechat.qr.qr_data_url));
  strlcpy(s_wechat.qr.status, "waiting_scan", sizeof(s_wechat.qr.status));
  strlcpy(s_wechat.qr.message, "使用微信扫描二维码完成登录。",
          sizeof(s_wechat.qr.message));
  s_wechat.qr.started_at_ms = cap_im_wechat_now_ms();
  strlcpy(s_wechat.qr.current_api_base_url, CAP_IM_WECHAT_DEFAULT_BASE_URL,
...
static esp_err_t cap_im_wechat_qr_poll_once_locked(void) {
  ...
  snprintf(endpoint, sizeof(endpoint), "ilink/bot/get_qrcode_status?qrcode=%s",
           s_wechat.qr.qrcode);
  err = cap_im_wechat_http_get_json(
      s_wechat.qr.current_api_base_url[0] ? s_wechat.qr.current_api_base_url
                                          : CAP_IM_WECHAT_DEFAULT_BASE_URL,
      endpoint, CAP_IM_WECHAT_QR_POLL_TIMEOUT_MS, &response);
  if (err != ESP_OK) {
    cap_im_wechat_resp_cleanup(&response);
    return err;
  }
  ...
  status = cap_im_wechat_string_value(
      cJSON_GetObjectItemCaseSensitive(root, "status"));
  redirect_host = cap_im_wechat_string_value(
      cJSON_GetObjectItemCaseSensitive(root, "redirect_host"));
  bot_token = cap_im_wechat_string_value(
      cJSON_GetObjectItemCaseSensitive(root, "bot_token"));
  ilink_bot_id = cap_im_wechat_string_value(
      cJSON_GetObjectItemCaseSensitive(root, "ilink_bot_id"));
  ilink_user_id = cap_im_wechat_string_value(
      cJSON_GetObjectItemCaseSensitive(root, "ilink_user_id"));
  baseurl = cap_im_wechat_string_value(
  ...
  if (strcmp(status, "wait") == 0) {
    ...
    if (ilink_bot_id) {
      strlcpy(s_wechat.qr.ilink_bot_id, ilink_bot_id,
              sizeof(s_wechat.qr.ilink_bot_id));
    }
    if (ilink_user_id) {
      strlcpy(s_wechat.qr.ilink_user_id, ilink_user_id,
              sizeof(s_wechat.qr.ilink_user_id));
    }
    if (baseurl && baseurl[0]) {
      strlcpy(s_wechat.qr.base_url, baseurl, sizeof(s_wechat.qr.base_url));
    } else {
      strlcpy(s_wechat.qr.base_url,
              s_wechat.qr.current_api_base_url[0]
    ...
    strlcpy(s_wechat.qr.status, "error", sizeof(s_wechat.qr.status));
    strlcpy(s_wechat.qr.message, "二维码状态未知。",
            sizeof(s_wechat.qr.message));
  }
...
static void cap_im_wechat_qr_task(void *arg) {
  ...
  while (1) {
    ...
    if (cap_im_wechat_now_ms() - s_wechat.qr.started_at_ms >
        CAP_IM_WECHAT_QR_TTL_MS) {
      if (s_wechat.qr.refresh_count + 1 >= CAP_IM_WECHAT_QR_MAX_REFRESH) {
        strlcpy(s_wechat.qr.status, "expired", sizeof(s_wechat.qr.status));
        strlcpy(s_wechat.qr.message, "二维码已多次过期，请重新生成。",
                sizeof(s_wechat.qr.message));
        s_wechat.qr.active = false;
        s_wechat.qr_task = NULL;
        cap_im_wechat_unlock();
      ...
      if (err != ESP_OK) {
        if (cap_im_wechat_lock() == ESP_OK) {
          strlcpy(s_wechat.qr.status, "error", sizeof(s_wechat.qr.status));
          strlcpy(s_wechat.qr.message, "刷新二维码失败。",
                  sizeof(s_wechat.qr.message));
          s_wechat.qr.active = false;
          s_wechat.qr_task = NULL;
          cap_im_wechat_unlock();
    ...
    if (err == ESP_ERR_TIMEOUT) {
      ...
      strlcpy(s_wechat.qr.status, "error", sizeof(s_wechat.qr.status));
      snprintf(s_wechat.qr.message, sizeof(s_wechat.qr.message),
               "轮询扫码状态失败: %s", esp_err_to_name(err));
      s_wechat.qr.active = false;
      should_break = true;
